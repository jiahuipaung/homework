...
func Test_UserCase_Convert_Handle(t *testing.T) {
	type args struct {
		ctx   context.Context
		input string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			convert, err := ConvertJsonPruneV2ToTransformSettings(tt.prune)
			if err != nil {
				t.Fatal(err)
			}
			...
			got, err := transformer.Handle(tt.args.ctx, &types.LogEvent{
				Message: tt.args.input,
			})
			if (err != nil) != tt.wantErr {
				t.Errorf("test %s Handle() error = %v, wantErr %v", tt.name, err, tt.wantErr)
				return
			}
			...
			if utils.MustToJsonString(got) != tt.want {
				t.Errorf("test %s Handle() got = %v, want %v", tt.name, got, tt.want)
				return
			}
		})
	}
